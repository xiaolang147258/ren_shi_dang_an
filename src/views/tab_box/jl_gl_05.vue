<template>
  ...
  <div class="stu" style="position: relative;">
       <div style="width: 100%;height:56px;border-bottom:1px solid #ECECEC;">
       		 <p style="font-size:28px;font-weight:600;float:left;">奖励管理</p>
			 <el-button @click='git_fuchad2("",false)' type="primary" style="float:right;">新增员工奖励</el-button>
       </div>
	   ...
	   <el-table v-loading='loading' :data="tableData" width='100%' height='561' stripe style="width: 100%" :height='670'>
	     ...
	     <el-table-column header-align='center' align='center' prop="staff_name" label="奖励对象" ></el-table-column>
		 <el-table-column header-align='center' align='center' prop="name" label="奖励方式">
			 <template slot-scope="scope">{{scope.row.type==1?'奖励':'惩罚'}}</template>
		 </el-table-column>
		 <el-table-column header-align='center' align='center' prop="desc" label="奖励说明"  show-overflow-tooltip></el-table-column>
	  ...
	  <!-- 分页 -->
		 ...
		 <el-dialog v-loading="loading" :visible.sync="dialogVisible2" width="35%" :close-on-click-modal='false' :before-close="handleClose">
		     <p class="pbox" style="text-align: center;width:100%;font-size:23px;height:40px;">{{act_yuan?'修改员工奖励':'新增员工奖励'}} </p>
			 <div class="tabboxa">
...
export default {
  ...
  methods:{
	  ...
	  delect(i){
		this.$confirm('确定删除 '+i.staff_name+'的这条奖励记录吗?', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning',
		  }).then(()=>{
			  this.loading = true;
...
//离职处理
  ...
  mounted(){
	  this.indexof(this.$router.options.routes[1].children,'奖励管理');
	  this.git_active();//获取数据
	  this.git_acts()
  }
}
